...
public class MappersTest {
    ...
    private Long createAgent(String name) {
        Agent agent = Agent.builder()
            .name(name).description("for fk").avatar("a").status("draft")
            .prompt("p").category("c").adminId(1L).userId(1L).tags("t")
    ...
    public void testAgentMapper() {
        ...
        agentMapper.findAll(null).stream().map(Agent::getId).forEach(id -> agentMapper.deleteById(id));
        List<Agent> all = agentMapper.findAll(null);
        Assertions.assertEquals(List.of(), all);
        Agent agent = Agent.builder()
            .name("test").description("test").avatar("test").status("test")
            .prompt("test").category("test").adminId(1L).userId(1L).tags("test")
            .createTime(LocalDateTime.now().withNano(0))
            .updateTime(LocalDateTime.now().withNano(0))
            .build();
        int insert = agentMapper.insert(agent);
        Assertions.assertEquals(1, insert);
        Agent findById = agentMapper.findById(agent.getId(), null);
        Assertions.assertEquals(agent, findById);
        List<Agent> findByStatus = agentMapper.findByStatus("test", null);
        Assertions.assertEquals(List.of(agent), findByStatus);
        List<Agent> searchByKeyword = agentMapper.searchByKeyword("test", null);
        ...
        List<Agent> allAfterDelete = agentMapper.findAll(null);
